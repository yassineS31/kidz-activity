...
import {MdPermIdentity , MdGroups,MdExplore,MdOutlineArticle,MdRecommend ,MdLogout,MdEventNote,MdHome, } from "react-icons/md";
...
const toggleSidebar = () => {
    ...
    return (
        ...
        <div className='w-full flex items-center justify-between'>
            <h2 className='font-bold my-5 text-center w-full'>Navigation</h2>
            <svg
        ...
        <ul className='flex flex-col items-center '>
            <li >
                <Link href="/" className='my-3 text-emerald-800 font-bold flex items-center gap-2'>
                    <MdHome className={`${isCollapsed ? 'justify-self-center size-9' : ''}`}/>
                    <span className={`${isCollapsed ? 'hidden' : 'inline'}`}>Accueil</span> 
                </Link>
            </li>
            <li>
                <Link href={"/ActivitiesList"} className='my-3 flex items-center gap-2'>
                    <svg width="40" height="40" className={`${isCollapsed ? 'justify-self-center size-9' : 'icon'}`} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M21.25 15.7V5.9875C21.25 4.19636 19.8036 2.75 18.0125 2.75H5.9875C4.19636 2.75 2.75 4.19636 2.75 5.9875V18.0125C2.75 19.8036 4.19636 21.25 5.9875 21.25H15.7L21.25 15.7ZM5.9875 19.4C5.22227 19.4 4.6 18.7777 4.6 18.0125V5.9875C4.6 5.22227 5.22227 4.6 5.9875 4.6H18.0125C18.7777 4.6 19.4 5.22227 19.4 5.9875V14.775H18.0125C16.2214 14.775 14.775 16.2214 14.775 18.0125V19.4H5.9875ZM9.93136 9.94818C9.6875 9.09045 8.79614 8.58591 7.93841 8.82977C7.08068 9.07364 6.57614 9.965 6.82 10.8311C6.88727 11.0666 7.005 11.2768 7.15636 11.4534L9.965 10.663C10.007 10.4275 9.99864 10.1836 9.93136 9.94818ZM15.7168 8.31682C15.4814 7.45909 14.5816 6.95455 13.7239 7.19841C12.8661 7.44227 12.3616 8.33364 12.6055 9.19977C12.6727 9.43523 12.7905 9.64545 12.9418 9.82205L15.7505 9.03159C15.7925 8.79614 15.7841 8.55227 15.7168 8.31682ZM16.5409 11.2095L7.62727 13.7239C8.82977 15.3132 10.9152 16.0868 12.9418 15.515C14.9684 14.9432 16.3391 13.1857 16.5409 11.2095Z" fill="#333333"/>
                    </svg>
                    <span className={`${isCollapsed ? 'hidden' : 'inline'}`}>Activité</span>
                </Link> 
            </li>
            <li>
                <Link href={'/Schedule'} className='my-3 flex items-center gap-2 '>
                    <MdEventNote  className={`${isCollapsed ? 'justify-self-center size-9' : ''}`}/>
                    <span className={`${isCollapsed ? 'hidden' : 'inline'}`}>Planning</span>
                </Link>
                
            </li>
            <li>
                <Link href="/Groups" className='my-3 flex items-center gap-2'>
                    <MdGroups className={`${isCollapsed ? 'justify-self-center size-9' : ''}`}/>
                    <span className={`${isCollapsed ? 'hidden' : 'inline'}`}>Groupes</span>
                </Link>
            </li>
            <li >
                <Link href="/Profil" className='my-3 flex items-center gap-2'>
                    <MdPermIdentity className={`${isCollapsed ? 'justify-self-center size-9' : ''}`}/>
                    <span className={`${isCollapsed ? 'hidden' : 'inline'}`}>Mon profil</span>
                </Link>
            </li>
            <li>
                <Link href="/MyChildrens" className='my-3 flex items-center gap-2'>
                    <FaChildren className={`${isCollapsed ? 'justify-self-center size-9' : ''}`}/>
                    <span className={`${isCollapsed ? 'hidden' : 'inline'}`}>Mes enfants</span>
                </Link>
            </li>
        </ul>
        <h2 className='font-bold my-6 flex items-center gap-2'> <MdExplore className={`${isCollapsed ? 'justify-self-center size-9' : ''}`} />
        <span className={`${isCollapsed ? 'hidden' : 'inline'}`}>Explorer</span></h2>
        <ul className='flex flex-col items-left'>
            <li>
                <Link href="/SharingMoments" className=' my-3 flex items-center gap-2'>
                    <TbPhotoHeart className={`${isCollapsed ? 'justify-self-center size-9' : ''}`}/>
                    <span className={`${isCollapsed ? 'hidden' : 'inline'}`}>Feed</span>
                </Link>
            </li>
            <li>
                <Link href="/Ranking" className=' my-3 flex items-center gap-2'>
                    <svg className={`${isCollapsed ? 'justify-self-center size-9' : 'icon'}`} width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <span className={`${isCollapsed ? 'hidden' : 'inline'}`}>Classement</span>
                </Link>
            </li>
            <li>
                <Link href="/Events" className=' my-3 flex items-center gap-2'>
                <IoCalendarOutline className={`${isCollapsed ? 'justify-self-center size-9' : ''}`}/>
                <span className={`${isCollapsed ? 'hidden' : 'inline'}`}>Événements</span>
                </Link> 
            </li>
            <li>
                <Link href="/BlogAndConseils" className=' my-3 flex items-center gap-2' >
                    <MdOutlineArticle className={`${isCollapsed ? 'justify-self-center size-9' : ''}`}/>
                    <span className={`${isCollapsed ? 'hidden' : 'inline'}`}>Blog</span>
                </Link> {/* Ajustez si nécessaire */}
            </li>
            <li>
                <Link href="/Recommendations" className=' my-3 flex items-center gap-2'>
                    <MdRecommend  className={`${isCollapsed ? 'justify-self-center size-9' : ''}`}/>
                    <span className={`${isCollapsed ? 'hidden' : 'inline'}`}>Pour vous</span>
                </Link>
            </li>
        </ul>
        <Link href="/Logout" className=' mt-32 flex items-center gap-2'>
            <MdLogout className={`${isCollapsed ? 'justify-self-center size-9' : ''}`}/>
            <span className={`${isCollapsed ? 'hidden' : 'inline'}`}>Déconnexion</span>
        </Link>
    </div>
);
};
