...
const CardActivity = ({ id, title, categorie,price,user,description,time,index }: { id: number; title: string; categorie: string;price:string;user:string;description:string;time:string,index:number }) => {
  const bgColor = bgColors[index % bgColors.length]; // Cyclique
  const textColor = textColors[index % textColors.length]; // Cyclique
    return (
<div className={`relative max-w-[250px] h-[400px] ${bgColor} border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700`}>
    ...
    <Image src={"/assets/images/museum.jpg"} alt="image profil" width={250} height={200} className="rounded-t-lg h-[200px]"></Image>
    <div className="p-4">
        ...
        <p className={`${textColor} my-3 text-sm font-normal text-gray-700 dark:text-gray-400`}>{description}</p>
